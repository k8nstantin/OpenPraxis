		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					cpu, rssMB, memUsed, memTotal, netRx, netTx, diskR, diskW, load := s.node.SystemSnapshot()
					row := execution.Row{
						ID:           uuid.Must(uuid.NewV7()).String(),
						RunUID:       cur.RunUID,
						EntityUID:    mcpSessionID,
...
func (s *Server) registerTools() {
	...
	s.mcp.AddTool(
		mcplib.NewTool("conversation_save",
			...
			mcplib.WithString("agent", mcplib.Description("Agent name: claude-code, gemini-cli, cursor, copilot, etc.")),
	...
	s.mcp.AddTool(
		mcplib.NewTool("comment_add",
			mcplib.WithDescription("Post a comment on a product, manifest, or task. Mirrors POST /api/{products|manifests|tasks}/{id}/comments. The runner's closing step uses this to record the comment."),
