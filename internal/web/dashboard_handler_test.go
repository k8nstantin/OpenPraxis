...
// Tests for serveDashboard — the Svelte v2 mount point at /dashboard/.
// Four code paths to cover:
//
...
//
// Plus the negative-space cases:
//	5. /dashboard/index.html (must not infinite-loop — that was the chunk-7 bug)
//	6. dist/ missing index.html → 500 with helpful message

func newTestDashboardFS() fstest.MapFS {
	return fstest.MapFS{
		"index.html":              {Data: []byte("<!doctype html><html>app</html>")},
		"assets/index-abc123.js":  {Data: []byte("console.log('svelte');")},
...
func TestServeDashboard_SpaRouteFallsBackToIndex(t *testing.T) {
	h := serveDashboard(newTestDashboardFS())
	// /dashboard/products/abc-route — no such file in the embed; the
	// Svelte router owns this path and serveDashboard must serve
	// index.html so a hard refresh of a deep link works.
...
func TestServeDashboard_ExplicitIndexHtmlNoRedirectLoop(t *testing.T) {
	...
	// every request → infinite 301 loop. This test fails fast if anyone
	// reintroduces the bug.
	h := serveDashboard(newTestDashboardFS())
	resp := doDashReq(t, h, "/dashboard/index.html")
	if resp.StatusCode == 301 || resp.StatusCode == 302 {
		t.Fatalf("status %d — index.html must not redirect (chunk-7 redirect-loop regression)", resp.StatusCode)
	}
	if resp.StatusCode != 200 {
		t.Fatalf("status: got %d, want 200", resp.StatusCode)
	}
...
func TestServeDashboard_RootLevelStaticFile(t *testing.T) {
	// Files like favicons that live at dist/<name> (not under
	// dist/assets/) get the conservative no-cache header — they're not
	// content-addressed so we can't safely long-cache them.
	h := serveDashboard(newTestDashboardFS())
...
func TestServeDashboard_MissingIndexReturns500(t *testing.T) {
	// If the embed somehow doesn't contain index.html, surface a clear
	// error instead of a confusing empty 200 or fileserver 404.
	...
	if !strings.Contains(body, "make build") {
		t.Errorf("body should hint at `make build`, got %q", body)
	}
}
