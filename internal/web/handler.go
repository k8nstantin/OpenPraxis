...
// React v2 dashboard build output. The `all:` prefix preserves the
...
func Handler(n *node.Node, mcpServer *mcp.Server, hub *Hub, peerRegistry *peer.Registry, chatRouter *chat.Router, chatCtx *chat.ContextBuilder, chatTools *chat.ChatTools) http.Handler {
	...
	// React v2 dashboard — embedded Vite build at /dashboard/. Hashed
	// assets land at /dashboard/assets/*; everything else under
	// /dashboard/ falls back to index.html so the React client-side
...
// serveDashboard serves the React v2 dashboard from the embedded Vite
// build. URL layout:
//
...
func serveDashboard(content fs.FS) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, req *http.Request) {
		...
		// serve index.html so the React router can take over. fs.Stat is
