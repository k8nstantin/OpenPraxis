// DAG view for a Product hierarchy. The reusable Cytoscape lifecycle
// wrapper now lives in `@/components/cy/CytoscapeCanvas`; this component
// just owns the DAG-specific stylesheet + element builder.
import { useMemo } from 'react';
import { buildDagElements } from '../lib/dag';
import type { ProductHierarchy } from '../lib/types';
import { CytoscapeCanvas, type CytoscapeStyle } from './cy/CytoscapeCanvas';
...
const DAG_STYLESHEET: CytoscapeStyle[] = [
  { selector: 'node', style: {
...
const DAG_LAYOUT = { name: 'dagre', rankDir: 'TB', nodeSep: 40, rankSep: 90, edgeSep: 25, padding: 32, fit: true };

export default function ProductDag({ data, onNodeClick }: Props) {
  const elements = useMemo(() => (data ? buildDagElements(data) : []), [data]);
  if (!data) return null;
  return (
    <CytoscapeCanvas
      elements={elements}
      stylesheet={DAG_STYLESHEET}
      layout={DAG_LAYOUT}
      className="product-dag-canvas"
      onNodeClick={onNodeClick ? (d) => onNodeClick(String(d.id ?? ''), String(d.type ?? '')) : undefined}
    />
  );
}
