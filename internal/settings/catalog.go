...
func Catalog() []KnobDef {
	return []KnobDef{
		...
		{Key: "prompt_prior_runs_limit", Type: KnobInt, SliderMin: f(0), SliderMax: f(20), SliderStep: f(1), Default: 5, Description: "How many prior execution-log rows to inject into the prior_context prompt section. 0 disables prior-run injection entirely."},
		{Key: "prompt_prior_comments_limit", Type: KnobInt, SliderMin: f(0), SliderMax: f(10), SliderStep: f(1), Default: 3, Description: "How many prior agent comments (type=comment) to inject into the prior_context prompt section. 0 disables comment injection."},
		{Key: "prompt_build_timeout_seconds", Type: KnobInt, SliderMin: f(1), SliderMax: f(30), SliderStep: f(1), Default: 5, Unit: "seconds", Description: "Deadline for the prompt-build history queries (prior runs + comments fetch). Prevents a slow DB from blocking task dispatch."},
