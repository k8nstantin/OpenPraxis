...
types:
	...
	@mkdir -p $(DASHBOARD_DIR)/src/lib/api/.tygo
	tygo generate --config tools/tygo/config.yaml
	@# tygo writes one file per package (last write wins on shared output_path),
	@# so Portal V2 packages emit to .tygo/*.ts staging files which we concat
	@# into the single generated.ts the dashboard imports.
	@printf '// Code generated by tygo. DO NOT EDIT.\n\n' > $(DASHBOARD_DIR)/src/lib/api/generated.ts
	@for f in execution entity action comments; do \
		if [ -f $(DASHBOARD_DIR)/src/lib/api/.tygo/$$f.ts ]; then \
			tail -n +2 $(DASHBOARD_DIR)/src/lib/api/.tygo/$$f.ts >> $(DASHBOARD_DIR)/src/lib/api/generated.ts; \
		fi; \
	done
	@rm -rf $(DASHBOARD_DIR)/src/lib/api/.tygo
